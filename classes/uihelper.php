...
class UIHelper
{
  ...
  protected function fixSVGSize($data, $file)
  {
    ...
    if ($xml) { // stolen from SVG Upload plugin
      $attr = $xml->attributes();
  ...
  public function featureNotice()
  {
    // @todo Remove in 2023.
    $message = sprintf(
      __('%s New Beta Feature! %s %s Enable Media Replace now gives you the ability to remove the background of any image. Try it out in the Media Library: hover over an image and click on Remove Background. Or just click on Remove background from the image editing window! %s  ', 'enable-media-replace'),
      '<h3>',
      '</h3>',
      '<p>',
      '</p>'
