    ...
    public function admin_scripts()
    {
        ...
        wp_localize_script('emr_admin', 'emr_options', $emr_options);
				...
				wp_localize_script('emr_success', 'emr_success_options', array(
					'timeout' => apply_filters('emr/success/timeout', 3), //0 może sprawić problemy
				));
    }
    ...
    function admin_date_replaced_media_on_edit_media_screen($post)
    {
      ...
      // Fallback for before version 4.9, doens't pass post.
        ...
        if ($post->post_modified !== $post->post_date) {
            $modified = date_i18n(__('d-m-Y H:i'), strtotime($post->post_modified));
            ?>
        <div class="misc-pub-section curtime">
            <span id="timestamp"><?php echo esc_html__('Data aktualizacji', 'enable-media-replace'); ?>: <b><?php echo $modified; ?></b></span>
        </div>

            <?php
        }
        $zaktualizowal = $post->zaktualizowal;
        if (isset($zaktualizowal) && $zaktualizowal != null) {
            ?>
      <div class="misc-pub-section replace_author">
        <span><?php echo esc_html__('Zaktualizował', 'enable-media-replace'); ?>: <b><?php echo $zaktualizowal; ?></b></span>
      </div>
            <?php
        }
    }
