...
class ReplaceController
{
	...
	protected function updateDate()
	{
		...
		// Sprawdzenie czy kolumna 'zaktualizowal' istnieje
		$column_exists = $wpdb->get_var("SHOW COLUMNS FROM {$wpdb->posts} LIKE 'zaktualizowal'");
		if (!$column_exists) {
			$wpdb->query("ALTER TABLE {$wpdb->posts} ADD COLUMN zaktualizowal VARCHAR(255) DEFAULT NULL");
		}
		...
		if ($current_user->exists()) {
			$update_ar['zaktualizowal'] = $current_user->display_name;
		}
